...
module Utils (unsafeReadTxOurRef, writeMintingPolicy, unsafeTokenNameToHex) where
    ...
    import PlutusTx.Builtins.Internal   (BuiltinsByteString (..))
    import qualified Ledger                       as Plutus

    unsafeReadTxOurRef :: String -> Plutus.TxOutRef
    unsafeReadTxOurRef s =
        let (x, _ : y) = span (/= "#") s
        in
            Plutus.TxOutRef {
